+++
title = "Links sobre física"
hascode = false
date = Date(2025, 12, 26)
rss = "Links sobre física. Útil para autoaprendizaje. Videos, tutoriales, todo lo que encuentre interesante"

tags = ["autoaprendizaje", "física"]
+++


# Links sobre física
El objetivo de este post es ir recolectando links útiles para el auto-aprendizaje de física. No solo para aprender cosas que todavía no se, sinó también para mis clases, siempre hay material interesante que aporta una nueva visión sobre algo que uno ya conoce. A medida que encuentre nuevo material lo voy a ir subiendo.

\toc

## General

* Curso Interactivo de Física en Internet (sin dudas, el mejor y mas completo): [http://www.sc.ehu.es/sbweb/fisica3/autor/descripcion.html](http://www.sc.ehu.es/sbweb/fisica3/autor/descripcion.html)